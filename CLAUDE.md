...
AI 机票预订工作台（TripDesk）：左聊天、右 bench 渲染订票领域状态。agent 走完整在线机票流程（搜索→验价→下单→支付→售后），数据来自 **travelkit-pro skill**（vendored 在 `.claude/skills/travelkit-pro`，python 脚本**直连 Simplifly Flight OpenAPI**，非 MCP）。产品定位/用户/原则见 **PRODUCT.md**；视觉设计系统（色板/字体/组件，kami house style）见 **DESIGN.md**。
...
- **领域状态机** `src/frames.ts`：把 relay 事件里 travelkit-pro 脚本的 compact JSON `tool_result`（Bash 结果，**按 shape 认**而非工具名）解析成 `stage`(search/verify/order/payment) + 各阶段数据 + `notice`。**UI 是工具结果的镜像**，agent 不额外写文件。
...
- `POST /v1/tasks` 走 **agent-loop**：manager 不自己跑机票流程，而是**委派**沙箱子 agent（`coding_agent__run_claude_code_in_sandbox`）去跑 travelkit-pro skill（`flight_search.py` 等直连 HTTP）；父任务事件流只见委派 + manager 散文总结。鉴权用 `API_KEY` 头（非 Bearer），client 见 `server/rebyte/client.ts`。
...
- 每用户一个沙箱，首轮在 DO 里按需 provision + seed travelkit-pro skill + 凭证 `.simplifly.env`（纯 fetch，不用 SDK；`worker/seed.ts`），存 `agent_computers` 表。
